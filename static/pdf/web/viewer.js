        ...
        var PDFViewerApplication = {
            ...
            _initializeViewerComponents: function _initializeViewerComponents() {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    // _this2.eventBus.on('resize', _this2._adjustWidth.bind(_this2));
                    // _this2.findBar = new PDFFindBar(findBarConfig, _this2.l10n);// 实例化PDFFindBar
                    // 这时我要是时点击keydown就会触发查找事件
                    // 同样的，我可以在这里直接触发查找函数
                    //aaaaaaaaaaaaaaaaaaaaaaaaa
//                    console.log(window.location.href);
                    if (window.location.href.indexOf("&") > -1) {
                        var aa = decodeURI(window.location.href.split("&")[1].split("=")[1]).split(",")
                    } else {
                        var aa = [];
                    }
//                    console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", aa)
                    _this2.highLightWords = aa;
                    /*console.log("aaaaaaaaaaaaaaaaaaaaaaaaa" + window.location.href);
                    _this2.highLightWords = ['的', "控制", "服务器"];*/
                    // wordHighLight(highLightWords)

                    //bbbbbbbbbbbbbb
                    (function wordHighLight(highLightWords) { // 目前只能匹配一个，不能全部高亮
//                        console.log(_this2.highLightWords.join(","))
                        let evt = {
                            // source: PDFFindBar, // PDFFindBar的实例，不确定是干嘛用的？
                            type: '',  // 这里默认应该是空的
                            // 这里能默认跳转到query的位置，刚好能满足要求
                            query: _this2.highLightWords, // 高亮的关键词 _this2.highLightWords.join(",")
            ...
            load: function load(pdfDocument, scale) {
                ...
                firstPagePromise.then(function (pdfPage) {
                    ...
                    Promise.all([storePromise, pageModePromise]).then(function (_ref2) {
                        ...
                        pdfViewer.update();
                    }).then(function () {//zlpzlpzlzpzlpzlpzzlp
//                        var pagedata = JSON.parse(localStorage.getItem('pagepdfdata'));
                        var pagedata = [{page:"5",keyweight:2},{page:"10",keyweight:2},{page:"15",keyweight:2},{page:"22",keyweight:2},{page:"24",keyweight:2},{page:"40",keyweight:2}]
                        var pageBox = document.getElementById("maskpage");
                        pagedata.map((item) => {
                            var pageDom = document.createElement("div");
                            pageDom.innerHTML = `<div class="pageitem">第${item.page}页</div><span>【${item.keyweight}】</span>`
                            pageBox.appendChild(pageDom)
                        });
                        var pageitem = document.getElementsByClassName("pageitem");
                        for (var i = 0; i < pageitem.length; i++) {
                            let index = i;
                            var self = window;
                            pageitem[index].onclick = function () {
                                pageitem[index].parentNode.style.backgroundColor = "black";
                                sibling(pageitem[index].parentNode);
                                var pagedetail = pageitem[index].innerHTML.replace("第", '').replace("页", '');
                                var pageNumber = document.getElementById("pageNumber");
                                pageNumber.value = pagedetail;

                                self.PDFViewerApplication.pdfViewer._currentPageNumber = pagedetail;
                                self.PDFViewerApplication.pdfViewer._location.pageNumber = pagedetail;
                                console.log(self.PDFViewerApplication.pdfViewer._currentPageNumber)
                                console.log(self.PDFViewerApplication.pdfViewer);
                                self.PDFViewerApplication.pdfViewer.update(1)
                            };
                        }
                        ...
                        function sibling(elm) {
                            ...
                            for (var i = 0, pl = p.length; i < pl; i++) {
                                if (p[i] !== elm) {
                                    a.push(p[i]);
                                    p[i].style.backgroundColor = "#626262";
                                }
                                ;
                            }
                            return a;
            ...
            setInitialView: function setInitialView(storedHash) {
                ...
                console.log("1847", this.pdfViewer.currentPageNumber)
...
//                    console.log("我试试试试试试",c_urlArray);
                    //默认缩放比例
...
//                eventBus.on('pageitem', ()=>{console.log(11111111111)});
        ...
        function webViewerFind(evt) {
            console.log("111111111webViewerFind", evt);
...
//                    console.log(query)
                    ...
                    /*console.log("dddddd", test_query, queryArray)*/
//                    console.log("dddddd")
                    for (let x = 0; x < test_query.length; x++) {
                        // var queryArray = test_query[x].match(/\S+/g);
              ...
              while (true) {
                ...
                value: function calcFindMatch(pageIndex) {
                    ...
                    var queryLen = query.length;
//                    console.log("ccccc", pageContent)
                    //ccccccccccccccccc
                    for (let i = 0; i < queryLen.length; i++) {
                        this.state.query[i] = this._normalize(this.state.query[i]);
                        ...
                        if (!caseSensitive) { // 判断是否对大小写敏感
                            ...
                            this.state.query[i] = this.state.query[i].toLowerCase();
                        }
                    }
                    //cccccccccccccccccc
                    if (queryLen === 0) {
        ...
        var PDFViewer = function () {
            ...
            _createClass(PDFViewer, [{
                ...
                value: function _setCurrentPageNumber(val) {
                    console.log("valvalvalval", val, val, val);
                ...
                value: function update(bool) {
                    ...
                    console.log(stillFullyVisible, visible)
                    console.log("9548", this._currentPageNumber, currentId);
                    //zlp start
                    if (!stillFullyVisible && !bool) {
                        currentId = visiblePages[0].id;
                    }
                    if (!this.isInPresentationMode && !bool) {
                ...
                value: function _getVisiblePages() {
                    console.log("9577this", this._currentPageNumber, this.isInPresentationMode)
                    if (!this.isInPresentationMode) {
        ...
        var Toolbar = function () {
            ...
            _createClass(Toolbar, [{
                ...
                value: function setPagesCount(pagesCount, hasPageLabels) {
                    console.log(pagesCount, pagesCount, pagesCount)
                    this.pagesCount = pagesCount;
        ...
        function getViewerConfiguration() {
            return {
                ...
                toolbar: {
                    ...
                    //zlp0612 start
                    topage: document.getElementsByClassName('pageitem'),
                    //zlp0612 end
                ...
                secondaryToolbar: {
                    ...
                    //zlp0612 start
//                    topageButton: document.getElementsByClassName('pageitem'),
                    //zlp0612 end
